...
<dict>
    <key>Label</key>
    <string>com.nanoclaw-opencode</string>
    ...
    <dict>
        <key>PATH</key>
        <string>/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:${homeDir}/.local/bin:${homeDir}/.volta/bin</string>
        <key>HOME</key>
        <string>${homeDir}</string>
    </dict>
    <key>StandardOutPath</key>
    <string>${projectRoot}/logs/nanoclaw-opencode.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/nanoclaw-opencode.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.nanoclaw-opencode');
  } catch {
...
StandardOutput=append:${projectRoot}/logs/nanoclaw-opencode.log
StandardError=append:${projectRoot}/logs/nanoclaw-opencode.error.log
...
function setupNohupFallback(
  ...
  const lines = [
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/nanoclaw-opencode.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nanoclaw-opencode.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nanoclaw-opencode.log"`,
  ];
