...
function buildVolumeMounts(
  ...
  // Per-group (and per-thread) OpenCode data directory
  // Each group/thread gets their own .opencode/ to prevent cross-group/thread session access
  const groupSessionsDir = threadKey
    ? path.join(DATA_DIR, 'sessions', group.folder, threadKey, '.opencode')
    : path.join(DATA_DIR, 'sessions', group.folder, '.opencode');
  fs.mkdirSync(groupSessionsDir, { recursive: true });

  // Sync skills from container/skills/ into each group's .opencode/skills/
  ...
  // Mount OpenCode data directory (DB, config, cache, state, skills)
  mounts.push({
    hostPath: groupSessionsDir,
    containerPath: '/home/bun/.opencode',
    readonly: false,
  });
...
/**
 * Read all .env vars for passing to the container via stdin.
 * Values are never written to disk or mounted as files.
 */
function readSecrets(): Record<string, string> {
  return readAllEnvFile();
}
...
function buildContainerArgs(
  ...
  // Redirect OpenCode's XDG paths into /home/bun/.opencode (the mounted dir)
  args.push('-e', 'XDG_DATA_HOME=/home/bun/.opencode');
  args.push('-e', 'XDG_CONFIG_HOME=/home/bun/.opencode');
  args.push('-e', 'XDG_STATE_HOME=/home/bun/.opencode');
  args.push('-e', 'XDG_CACHE_HOME=/home/bun/.opencode');
  args.push('-e', 'HOME=/home/bun');

  // Run as host user so bind-mounted files have matching ownership
...
export async function runContainerAgent(
  ...
  const threadSuffix = input.threadKey
    ...
    : '';
  const containerName = `nanoclaw-opencode-${safeName}${threadSuffix}-${Date.now()}`;
