...
export function getMessagesSince(
  ...
  // Fetch user messages only (bot messages excluded — agent sees them via session history).
  const sql = `
    SELECT id, chat_jid, sender, sender_name, content, timestamp, thread_ts AS threadTs
    FROM messages
...
/**
 * Get recent scheduled task output that is visible to chat (exclude_from_history = 0).
 * Used to inject task context into the chat prompt.
 */
export function getRecentTaskOutput(
  chatJid: string,
  sinceTimestamp: string,
): NewMessage[] {
  const sql = `
    ...
    FROM messages
    WHERE chat_jid = ? AND timestamp > ?
      AND is_scheduled_task = 1 AND COALESCE(exclude_from_history, 0) = 0
      AND content != '' AND content IS NOT NULL
    ORDER BY timestamp
  `;
  return db
...
export function updateTask(
  ...
  if (updates.exclude_from_history !== undefined) {
    fields.push('exclude_from_history = ?');
    values.push(updates.exclude_from_history ? 1 : 0);
  }
