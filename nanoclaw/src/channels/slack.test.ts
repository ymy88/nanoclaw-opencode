...
describe('SlackChannel', () => {
  ...
  // --- Reconnect resilience (queue handoff + connect timeout) ---

  describe('reconnect resilience', () => {
    it('takePendingMessages drains and clears the outgoing queue', async () => {
      const channel = new SlackChannel(createTestOpts());

      // Queue while disconnected
      await channel.sendMessage('slack:C0123456789', 'one');
      await channel.sendMessage('slack:C0123456789', 'two', {
        threadTs: '111.222',
      });

      const pending = channel.takePendingMessages();
      ...
      // Queue is now empty — a subsequent take returns nothing
      expect(channel.takePendingMessages()).toEqual([]);
    });

    it('sendImage queues an image when disconnected', async () => {
      const channel = new SlackChannel(createTestOpts());

      await channel.sendImage('slack:C0123456789', '/tmp/pic.png', 'a caption', {
        threadTs: '5.5',
      });
    ...
    it('sendImage queues (does not throw) when the upload fails', async () => {
      const channel = new SlackChannel(createTestOpts());
      await channel.connect();
      ...
      await expect(
        channel.sendImage('slack:C0123456789', '/tmp/pic.png'),
      ).resolves.toBeUndefined();
    ...
    it('restorePending flushes handed-off text messages when connected', async () => {
      const channel = new SlackChannel(createTestOpts());
      await channel.connect();

      await channel.restorePending([
        {
          kind: 'text',
          jid: 'slack:C0123456789',
          text: 'recovered',
    ...
    it('restorePending flushes handed-off images via filesUploadV2', async () => {
      const channel = new SlackChannel(createTestOpts());
      await channel.connect();
      ...
      expect(currentApp().client.filesUploadV2).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          file: '/tmp/selfie.png',
          initial_comment: 'morning',
          thread_ts: '7.7',
        }),
      );
    });

    it('restorePending sends recovered messages before newly queued ones', async () => {
      const channel = new SlackChannel(createTestOpts());
      await channel.connect();

      // A message queued on the new channel after the swap window
      channel.takePendingMessages(); // ensure empty
      await channel.restorePending([
        { kind: 'text', jid: 'slack:C0123456789', text: 'older-recovered' },
      ]);

      const calls = currentApp().client.chat.postMessage.mock.calls.map(
        (c: any[]) => c[0].text,
      );
      expect(calls[0]).toBe('older-recovered');
    });

    it('hands off a dead channel queue to a freshly connected channel', async () => {
      // Simulate the dead channel: messages queued while disconnected
      const dead = new SlackChannel(createTestOpts());
      await dead.sendMessage('slack:C0123456789', 'stranded-1');
      await dead.sendMessage('slack:C0123456789', 'stranded-2');

      // New channel connects, then receives the handoff
      const fresh = new SlackChannel(createTestOpts());
      await fresh.connect();
      const pending = dead.takePendingMessages();
      await fresh.restorePending(pending);

      const texts = currentApp().client.chat.postMessage.mock.calls.map(
        (c: any[]) => c[0].text,
      );
      expect(texts).toContain('stranded-1');
      expect(texts).toContain('stranded-2');
      // Nothing left stranded on the dead channel
      expect(dead.takePendingMessages()).toEqual([]);
    });

    it('connect() rejects when app.start() hangs past the timeout', async () => {
      vi.useFakeTimers();
      try {
        const channel = new SlackChannel(createTestOpts());
        // Make the handshake hang forever
        currentApp().start = () => new Promise(() => {});
